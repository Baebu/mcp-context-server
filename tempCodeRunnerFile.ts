...
function simulateConsentActivity(consentService: UserConsentService) {
 ...
 details: {
  ...
  testRequests.forEach((request, index) => {
    setTimeout(async () => {
      try {
        // Add missing required properties for ConsentRequest
 const fullRequest = {
 ...request, // Spread existing properties (details, reason, timeout)
          operation: request.details.command ? 'command_execute' as const : (request.details.path ? 'file_access' as const : 'unknown' as const),
 severity: request.details.command?.includes('sudo') ? 'high' as const : (request.details.path?.includes('/tmp') ? 'low' as const : 'medium' as const),};        const response = await consentService.requestConsent(fullRequest);
  ...
  setInterval(() => {
    // Ensure testRequests is not empty before accessing elements
    if (testRequests.length === 0) {
 return;
    }
    const randomRequest = testRequests[Math.floor(Math.random() * testRequests.length)]; // 'randomRequest' is not 'undefined' here
    const randomizedRequest = {
      ...randomRequest,
      details: {
        ...randomRequest.details,
        path: randomRequest.details.path + '-' + Date.now()
      } as any // Cast to any to allow adding 'path' potentially not in original type
    };
 // Add missing required properties for ConsentRequest
    const fullRandomRequest = {
 ...randomizedRequest,
 operation: randomizedRequest.details.command ? 'command_execute' as const : (randomizedRequest.details.path ? 'file_access' as const : 'unknown' as const),
 severity: randomizedRequest.details.command?.includes('sudo') ? 'high' as const : (randomizedRequest.details.path?.includes('/tmp') ? 'low' as const : 'medium' as const),};
