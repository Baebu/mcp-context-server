...
// Get Workspace Stats Tool
const getWorkspaceStatsSchema = z.object({
  workspaceId: z.string().optional().describe('Workspace ID (defaults to active workspace)')
});

@injectable()
export class GetWorkspaceStatsTool implements IMCPTool {
  name = 'get_workspace_stats';
  description = 'Get detailed statistics about a workspace';
  schema = getWorkspaceStatsSchema;

  async execute(params: z.infer<typeof getWorkspaceStatsSchema>, context: ToolContext): Promise<ToolResult> {
    const workspaceManager = context.container.get<IWorkspaceManager>('WorkspaceManager');
    ...
    try {
      ...
      const stats = await workspaceManager.getWorkspaceStats(workspaceId);
      const files = await workspaceManager.getWorkspaceFiles(workspaceId);

      // Calculate additional insights
      const fileTypes = new Map<string, number>();
      // let totalLines = 0; // Unused variable

      for (const file of files) {
        const ext = file.filePath.split('.').pop() || 'no-extension';
            ...
            text: JSON.stringify(
              {
                ...
                stats: {
                  ...
                  averageFileSize: stats.fileCount > 0 ? Math.round(stats.totalSize / stats.fileCount) : 0,
      ...
      context.logger.error({ error, params }, 'Failed to get workspace stats');
      throw error;
    }
  }
