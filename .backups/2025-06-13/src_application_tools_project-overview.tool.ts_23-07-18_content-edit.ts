...
const getProjectOverviewSchema = z.preprocess(
  (data: any) => ({
    path: data?.path,
    includeFileTree: data?.includeFileTree ?? true,
    includeStats: data?.includeStats ?? true,
    includeContext: data?.includeContext ?? true,
    includeGitInfo: data?.includeGitInfo ?? true,
    includePackageInfo: data?.includePackageInfo ?? true,
    includeTechnologies: data?.includeTechnologies ?? true,
    maxDepth: data?.maxDepth ?? 3,
    maxFiles: data?.maxFiles ?? 50,
    excludePatterns: data?.excludePatterns ?? ['node_modules', '.git', 'dist', 'build', '*.log']
  }),
  z.object({
    ...
    includePackageInfo: z.boolean(),
    includeTechnologies: z.boolean(),
...
@injectable()
export class GetProjectOverviewTool implements IMCPTool<ProjectOverviewParams> {
