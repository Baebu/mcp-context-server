...
const getSystemHealthSchema: z.ZodType<SystemHealthParams> = z.preprocess(
  (data: any) => ({
  ...
  z.object({
    category: z.enum(['server', 'database', 'filesystem', 'security', 'workspace', 'semantic', 'all']),
...
type SystemHealthParams = {
  category: 'all' | 'security' | 'database' | 'filesystem' | 'server' | 'workspace' | 'semantic';
  includeRecommendations: boolean;
  detailed: boolean;
};

@injectable()
export class GetSystemHealthTool implements IMCPTool<SystemHealthParams> {
